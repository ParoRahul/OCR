...
class solr:
    ...
    def request_spellCheck(self,listofWords):
        try:
           ...
           #URL='http://localhost:{}/solr/{}/select?q=*:*&spellcheck.q={}&spellcheck=on'.format(self.PORT,self.SPELLCHECK_CORE,main_query)
           #print(URL)
           proxies = { 'http': 'http://683898:Paramita@20@proxy.tcs.com:8080', 
                       'https': 'http://683898:Paramita@20@proxy.tcs.com:8080'}
           headers = {'content-type': "application/json" }
           URL='http://localhost:8983/solr/#/DEOProduct/query'
           print(URL)
           response  = requests.post(URL,proxies=proxies,
                                     #data=json.dumps({}),
                                     data=json.dumps({"q":"Text:'body'"}), 
                                     headers=headers)
                                     #timeout=100)
           print("---")
